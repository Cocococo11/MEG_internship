...
dlgOk = gui.Dlg(title='Do you want to change the classifier ?')
...
dlgChange = gui.Dlg(title='Classifier or nbSteps change')
...
if few_trials:
    nb_trials_before_short_break = 2 # 50
    nb_trials_before_long_break = 6 # 200
    max1_trials = 40 # 1200
    max2_trials = 50 # 1400
...
if CHOICE_OF_EXPERIMENT == 'S2_with':
    ...
    MEGB.configure(nb_steps_chosen =nbSteps_chosen,clf_name =clfname,run_nbr = run_nbr,subjectId = participant )
    ...
    if (trials.thisN % nb_trials_before_short_break) == 0 and trials.thisN != 0:
        ...
        if(CHOICE_OF_EXPERIMENT=='S2_with'):
            ...
            if dlgOk.OK :# and (expOk['OK']==True ): 
                expChange['clf'],expChange['nbSteps'] =dlgChange.show()
                if dlgChange.OK and (expChange['clf'] != clfname or expChange['nbSteps']!=nbSteps_chosen):
                    ...
                    nbSteps_chosen = expInfo['nbSteps']
                    clfname = expInfo['classifier']
                    MEGB.configure(nb_steps_chosen =expChange['nbSteps'],clf_name =expChange['clf'],run_nbr = run_nbr,subjectId = participant )
