...
class MEGBuffer_Thread(QtCore.QThread):
    ...
    def run(self):
        ...
        self.subjectId = '0991'
        classifier = load('classifiers/'+self.subjectId+'_meg_CLF [-0.3,-0.1].joblib')
        lastIndex = None
        nbSteps = 4
    ...
    def stop(self):
        ...
        print("Saving data ...")
        savingFileName = 'saves/savedData'+self.subjectId +'_' + timeStamp + '.csv'
        matSaveData = np.c_[self.matSaveData,self.matSaveNbSamples,self.matDetect,self.matProbas,self.matProbas2]
        ...
        predicting_clf(timeStamp,self.subjectId)
